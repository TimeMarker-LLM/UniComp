...
class SigLipAttention(nn.Module):
    ...
    def forward(
        ...
        if wrapper in ["visionzip","unicomp"]:
        ...
        elif wrapper in ["unicomp"]:
            return attn_output, None, raw_key_states.mean(1)
        else:
...
class SigLipEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states, attn_weights, key = self.self_attn(
    ...
    def forward(
        ...
        for layer_idx, encoder_layer in enumerate(self.layers):
            if output_hidden_states:
                if wrapper in ["visionzip", "holitom", "unicomp"]:
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            if wrapper in ["visionzip"]:
                ...
                key = self.vision_tower.vision_model.encoder.layers[-1].key
                # query = self.vision_tower.vision_model.encoder.layers[-1].query

                return hidden_states, attn_weights.mean(dim=1).mean(dim=1), key, images.dtype
            if wrapper in ["unicomp"]:
                image_forward_outs = self.vision_tower(images.to(device=self.device, dtype=self.dtype), output_hidden_states=True, output_attentions=False)
                hidden_states = image_forward_outs.hidden_states[-1]
                key = self.vision_tower.vision_model.encoder.layers[-1].key
                return hidden_states, None, key, images.dtype
            ...
            #     attn_weights  = image_forward_outs.attentions[-1]
            #     hidden_states = image_forward_outs.hidden_states[-1]
            #     return hidden_states, attn_weights.mean(dim=1).mean(dim=1)
            else:
...
class SigLipVisionAbstract(nn.Module):
    ...
    def forward(self, images):
        last_hidden_state = self.vision_abstract.vision_model.post_layernorm(images.to(next(self.vision_abstract.parameters()).device))
